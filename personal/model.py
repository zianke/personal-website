...
def check_password(username, password):
    ...
    password_db_string = get_password_db_string(password)
    cur = get_db().execute(
    ...
    if not result:
        return 0
    return result['password'] == password_db_string
...
def change_password(user):
    ...
    cur = get_db().execute(
        "SELECT password FROM users WHERE username = '{}'".format(
            user['username']))
    if get_password_db_string(user['password']) != \
            cur.fetchone()['password']:
        cur.close()
        return 403
